...
#criterion = nn.L1Loss()#_mae
# 1. 加上 VGG feature extractor
...
class VGGFeatureExtractor(nn.Module):
    def __init__(self):
        ...
        self.slice = nn.Sequential(*list(vgg.children())[:16])  # 使用前 16 層（包含到 relu_2_2）
...
# 2. 寫一個 PerceptualLoss 類別
class PerceptualLoss(nn.Module):
    def __init__(self, feature_extractor, loss_fn=nn.MSELoss()):
        super().__init__()
        self.feature_extractor = feature_extractor
        self.loss_fn = loss_fn

    def forward(self, output, target):
        f_output = self.feature_extractor(output)
        f_target = self.feature_extractor(target)
        return self.loss_fn(f_output, f_target)

# 3. 把它當作 criterion 用起來
criterion = PerceptualLoss(feature_extractor, nn.MSELoss())
...
val_dataloader = test_loader  # 改這裡


for epoch in tqdm(range(num_epochs), desc="Epochs"):
    train_loss, train_psnr_val = train(model, train_dataloader, optimizer, criterion, device)
    val_loss, val_psnr_val = validate(model, val_dataloader, criterion, device)
